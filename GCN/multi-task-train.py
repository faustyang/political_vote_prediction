...
import random
from utils import *
from multi_task_model  import GCN
...
placeholders = {
    ...
    'vote_rate': tf.placeholder(tf.float32,shape=(1,3))
...
def evaluate(description, support, labels, mask, placeholders):
    t_test = time.time()
    feed_dict_val = multi_task_construct_feed_dic(description, support, labels, mask, placeholders)
    outs_val = sess.run([model.loss, model.accuracy,model.mlse], feed_dict=feed_dict_val)
...
test_vote=vote_list[0:470]
train_vote=vote_list[470:]
...
for item in train_list:
    ...
    for epoch in range(FLAGS.epochs):
        ...
        #print(description)
        feed_dict = multi_task_construct_feed_dic(description, support, y, y_mask, placeholders)
        feed_dict.update({placeholders['dropout']: FLAGS.dropout})
        outs = sess.run([model.opt_ops, model.loss, model.accuracy,model.vote_rate,model.voteoutputs,model.mlse], feed_dict=feed_dict)
        cost, acc,mse, duration = evaluate(description, support, y, y_val_mask, placeholders)
        cost_val.append(cost)
        vote_rate=str(outs[3])
        voteoutputs=str(outs[4])
        # Print results

        print("Epoch:", '%04d' % (epoch + 1),
              "mse=","{:.5f}".format(outs[5]),
              "val_acc=", "{:.5f}".format(acc), "time=", "{:.5f}".format(time.time() - t),
              "vote_rate=", "{0}".format(vote_rate),"voteoutputs=","{0}".format(voteoutputs)
              )
        # Training step
        outs = sess.run([model.opt_op, model.loss, model.accuracy,model.vote_rate,model.voteoutputs,model.mlse], feed_dict=feed_dict)
    for epoch in range(FLAGS.epochs):
    ...
    print("Test set results:", "cost=", "{:.5f}".format(test_cost_all),
         " #accuracy=", "{:.5f}".format(test_acc_all), "time=", "{:.5f}".format(test_duration_all),
          "mse=","{:.5f}".format(test_mse_all)
          )
    accuracy_test.append(test_acc_all)
    mse_list.append(test_mse_all)
mse_final=np.array(mse_list)
score_mse=mse_final.sum()/len(mse_list)
...
print("cross_validation result:","mse=","{:,f}".format(score_mse))
