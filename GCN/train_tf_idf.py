...
import random
from utils_tf_idf import *
from model_tfidf import GCN
...
flags.DEFINE_string('text_model', 'tfidf', 'method')
#flags.DEFINE_integer("all_legislation",475,"number of the legislation")
...
sample_description,y,y_mask=vote_vector('100',member_vote,member_idx)
...
placeholders = {
    ...
    'tfidf_feature': tf.placeholder(tf.float32, shape=(len(member_vote),FLAGS.feature_dim)),
    "vote_number":tf.placeholder(tf.int32)
}
# Create model
...
def evaluate(description, support, labels, mask, placeholders,X,vote_number):
    t_test = time.time()
    feed_dict_val = idf_construct_feed_dict(description, support, labels, mask, placeholders, X,vote_number)
...
for item in train_list:
    ...
    for epoch in range(FLAGS.epochs):
        ...
        #print(description)
        vote_number=item
        feed_dict = idf_construct_feed_dict(description, support, y, y_mask, placeholders,X,vote_number)
        ...
        # Validation
        cost, acc, duration = evaluate(description, support, y, y_val_mask, placeholders,X,vote_number)
    ...
    for item in test_vote:
        ...
        description.shape=(FLAGS.time_step,1)
        description=np.transpose(description)
        test_cost, test_acc, test_duration = evaluate(description, support, y, y_mask, placeholders,X,item)
