...
def construct_feed_dict(description, support, labels, labels_mask, placeholders):
...
def pretrain_construct_feed_dict(description, support, labels, labels_mask,memberfeature ,placeholders):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({placeholders['labels']: labels})
    feed_dict.update({placeholders['labels_mask']: labels_mask})
    feed_dict.update({placeholders['description']: description})
    feed_dict.update({placeholders['support'][i]: support[i] for i in range(len(support))})
    feed_dict.update({placeholders['memberfeature']: memberfeature})
    #feed_dict.update({placeholders['tfidf_feature']:tf_idf})
    #feed_dict.update({placeholders['vote_number']: vote_number})
    #feed_dict.update({placeholders['num_features_nonzero']: features[1].shape})
    return feed_dict
